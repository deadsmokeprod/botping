...
META: dict[str, dict[str, str]] = {
    "check_interval_sec": {
        ...
        "body": (
            "Как часто запускается полный цикл опроса всех включённых ботов (один круг getMe). "
    ...
    "request_timeout_sec": {
        "title": "Таймаут одного запроса getMe (секунды)",
        "body": (
            "Сколько максимум ждать ответ API на один запрос. Если Telegram не ответил за это время — "
            "проверка считается неуспешной (таймаут). Полезно при «подвисающей» сети."
        ),
        "etalon": "15",
    ...
    "fail_threshold": {
        ...
        "body": (
            "Сколько подряд неудачных проверок подряд нужно, чтобы открыть инцидент и отправить алерт «бот недоступен». "
            "Единица — чувствительно (один обрыв = алерт), 2–3 — меньше ложных срабатываний."
        ),
        "etalon": "2",
    ...
    "slow_ms": {
        ...
        "body": (
            "Если getMe успешен, но ответ дольше этого времени — в статусе/логе видно медленную проверку. "
