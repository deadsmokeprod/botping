...
| **⚙️ Настройки** | Группы параметров (дефолты), тихие часы, диск, **🏷 Наименования** |
...
### Индивидуальные настройки и алерты

**⚙️ Настройки** в главном меню задают **общие** пороги (интервал оценки, таймаут heartbeat, порог падений, восстановление, задержка первого алерта, повтор, медленный ответ).

В карточке **каждого** бота, роутера, LAN-устройства, сайта или модуля — кнопка **⚙️ Настройки**:

- те же параметры мониторинга, но только для этого объекта;
- пусто / «Сбросить к общим» — берутся глобальные значения;
- логика алертов как у проверки **Telegram API**: несколько плохих тиков → инцидент, задержка перед первым сообщением, несколько хороших тиков → «Восстановлено».

**Пример (модуль с дребезгом `tg_down`, без правки агента на сервере):**

1. **🌍 Сайты** → сайт → модуль «Telegram chat-bot form» → **⚙️ Настройки** → **Мониторинг и алерты**.
2. Задать только в Botping, например: порог падений **5–8**, задержка алерта down **600** с, порог восстановления **3**.
3. Краткие `ok: false` перестанут сразу слать пары «Недоступен / Восстановлено»; реальный простой по-прежнему дойдёт до алерта, но позже.

Скрипты на сайтах и MikroTik **менять не нужно** — меняется только обработка на VPS.
